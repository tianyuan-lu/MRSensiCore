...
mr_sens_z_normalized <- function(harmonized_data, K = 5, method_list = c("mr_ivw","mr_simple_mode","mr_weighted_mode","mr_weighted_median","mr_egger_regression")) {
  ...
  harmonized_data$instrument_group = (K + 1 - as.numeric(cut_number(abs(harmonized_data$beta.exposure/harmonized_data$se.exposure/sqrt(harmonized_data$samplesize.exposure)),K)))
  if ("chr.exposure" %in% colnames(harmonized_data) & "pos.exposure" %in% colnames(harmonized_data)) {
    instrument_group = data.frame(instrument = harmonized_data$SNP,
                                  ...
                                  z.exposure = harmonized_data$beta.exposure / harmonized_data$se.exposure / sqrt(harmonized_data$samplesize.exposure),
