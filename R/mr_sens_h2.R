...
#'
#' Rank core instruments by significance (per-variant heritability)
#'
...
mr_sens_h2 <- function(harmonized_data, K = 5, method_list = c("mr_ivw","mr_simple_mode","mr_weighted_mode","mr_weighted_median","mr_egger_regression")) {
  ...
  harmonized_data$instrument_group = (K + 1 - as.numeric(cut_number(harmonized_data$beta.exposure**2 * harmonized_data$eaf.exposure * (1 - harmonized_data$eaf.exposure),K)))
  if ("chr.exposure" %in% colnames(harmonized_data) & "pos.exposure" %in% colnames(harmonized_data)) {
    instrument_group = data.frame(instrument = harmonized_data$SNP,
                                  ...
                                  h2 = 2 * harmonized_data$beta.exposure**2 * harmonized_data$eaf.exposure * (1 - harmonized_data$eaf.exposure),
